...
→ **회귀는 곧 보너스 0점이다.** 기존 12 assertion 보존이 이 설계의 최상위 제약이다.

다만 그 제약이 화면 공간 문제는 **아니다.** §9에서 산술로 보이듯 셸은 화면을 두고
경쟁하지 않으며, mandatory 코드와 테스트는 손대지 않는다.
...
| selftest 범위 | **순수 함수만** (파서·토크나이저). 명령 실행은 하지 않음 | **계층 분리.** 파서 검증에 화면 출력이 끼면 실패했을 때 "파싱이 틀린 건지 출력이 틀린 건지"가 섞인다. 실행 경로는 계층 2가 덮는다 (§10). (화면 공간 때문이 아니다 — §9) |
| mandatory 코드·테스트 | **손대지 않음** | 셸이 화면을 두고 경쟁하지 않으므로 조정할 이유가 없다(§9). VI장이 mandatory PERFECT를 요구하는 이상, 필수도 아닌 미관을 위해 통과 중인 부분을 건드리는 것은 손익이 안 맞는다 |
...
**시작 배너를 찍지 않는다.** 화면이 모자라서가 아니라(§9 — 여유는 13행이다)
필요가 없어서다. 프롬프트 자체가 셸이 떴다는 신호이고, 발견성은 `help`가 담당한다.
넣고 싶어지면 넣어도 무방하다 — SCRL이 한 줄 더 밀려날 뿐 검사하는 것에 닿지 않는다.
...
보존이 최상위 제약이다. 다만 그 제약은 **화면 공간 문제가 아니다.** 산술을 해보면
그렇다.

VGA는 언제나 **마지막 25행**을 보여준다(`scroll()`이 오래된 행부터 밀어낸다).
`scroll_exercise` 이후의 출력은 고정 11행이고, 셸 프롬프트를 더해 12행이다.

```
보이는 SCRL 행 수 = 25 − 12 = 13행    (SCRL을 14행 이상 찍는 한 언제나 일정)
```

| 찍는 SCRL 행 | 총 출력 | 보이는 SCRL | `!SCRL00` |
|---|---|---|---|
| 30 (현재) | 42 | SCRL17~29 = 13행 | 통과 |
| 20 | 32 | SCRL07~19 = 13행 | 통과 |
| 14 | 26 | SCRL01~13 = 13행 | 통과 (여유 1행) |
| 13 | 25 | SCRL00~12 = 13행 | **실패** — 스크롤이 아예 안 일어남 |

여기서 두 가지가 따라 나온다.

**첫째, 셸은 화면을 두고 경쟁하지 않는다.** 셸 출력은 언제나 가장 최신이라 항상
보이고, 밀려나는 것은 언제나 가장 오래된 SCRL이다. 우리가 검사하는 것 중 거기 사는
것은 없다 — `SCRL29`는 보이는 13행 안에 있고, `!SCRL00`은 애초에 밀려나기를 *원하는*
검사다. 실제 여유는 **13행**이다. 프롬프트 한 줄은 SCRL을 한 줄 더 밀어낼 뿐이다.

**둘째, `scroll_exercise`를 줄여도 화면은 넓어지지 않는다.** 보이는 SCRL은 항상
13행으로 고정이고, 13행 밑으로 줄이면 스크롤 자체가 일어나지 않아 `!SCRL00`이
깨진다. 총 출력만 줄 뿐 얻는 것이 없다.

따라서 **mandatory 코드와 테스트는 손대지 않는다.** 화면 미관을 이유로 통과 중인
필수 부분을 건드리는 것은, VI장의 PERFECT 요구를 생각하면 손익이 맞지 않는다.

**부팅 화면이 SCRL로 절반 차는 것이 디펜스에서 걸린다면 `clear`가 푼다.**
`clear` → `gdt` 하면 KFS-2 내용만 있는 화면이 나온다. 부팅 화면을 영구히 바꾸지
않고도 원하는 그림을 만들 수 있고, 부팅 로그와 깨끗한 화면 **양쪽 다** 보여줄 수
있다는 점에서 오히려 낫다. 남아 있는 SCRL 13행도 디펜스 재료다 — "30줄을 찍었는데
13줄만 보인다, 나머지는 스크롤로 사라졌고 테스트가 `SCRL00`의 부재를 확인한다."

> 위 산술은 첫 태스크에서 `make test`로 검증한다. 12 assertion이 그대로 통과하면
> 맞은 것이다. 어긋나면 그때 재검토한다 — 다만 어긋날 경우에도 대응은 셸 쪽에서
> 하지 mandatory를 조정하지 않는다.
...
`selftest.c`에 `test_shell_parse()`를 추가한다. **명령을 실행하지 않는다** — 화면이
모자라서가 아니라(§9) **계층을 섞지 않기 위해서**다. 파서 검증에 화면 출력이 끼면
실패했을 때 파싱이 틀린 것인지 출력이 틀린 것인지 구분되지 않는다. 실행 경로는
계층 2가 덮는다. 여기서는 화면에 아무것도 남기지 않는 순수 함수만 검사한다.
...
- **화면 산술** — §9. 여유 13행이라는 계산은 첫 태스크에서 `make test`로 확인한다.
  틀렸더라도 대응은 셸 쪽에서 하며 mandatory는 조정하지 않는다
